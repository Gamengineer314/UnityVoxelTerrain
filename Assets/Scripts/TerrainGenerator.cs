...
{
    private const float amplitude = 30;
    private const float blockPeriod = 500;
    private const int idHeight = 18;

    public (NativeList<int> IDs, NativeArray<int> IDIndexes) GenerateTerrain()
    {
        NativeArray<int> heightMap = new NativeArray<int>(WorldManager.horizontalSize * WorldManager.horizontalSize, Allocator.Temp);
        NativeArray<int> ids = new NativeArray<int>(WorldManager.horizontalSize * WorldManager.horizontalSize, Allocator.Temp);
            ...
            for (int chunkX = 0; chunkX < WorldManager.horizontalChunks; chunkX++)
            {
                for (int zInChunk = 0; zInChunk < WorldManager.chunkSize; zInChunk++)
                {
                    for (int xInChunk = 0; xInChunk < WorldManager.chunkSize; xInChunk++)
                    {
                        ...
                        int y = heightMap[x + z * WorldManager.horizontalSize] - 1;
